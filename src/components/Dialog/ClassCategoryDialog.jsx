...
const ClassCategoryDialog = ({
  ...
  const handleAddNewCategory = async () => {
    ...
    try {
      ...
      const response = await api.post("/class/add-category", {
        name: newCategory,
      });
      getListCategory();
      setNewCategory("");
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const response = await api.put(
        `/class/edit-category/${selectedItem.id}`,
        {
  ...
  const handleDeleteClick = async (id) => {
    try {
      // eslint-disable-next-line no-unused-vars
      const response = await api.delete(`/class/delete-category/${id}`);
      getListCategory();
      setTypeAlert("success");
      setContentAlert("Category deleted successfully!");
      setOpenAlert(true);
      if (id === selectedCategory) setSelectedCategory("not-select");
    } catch (err) {
      setTypeAlert("error");
      setContentAlert(err.response.data.error);
      setOpenAlert(true);
        ...
        {data.map((category) => (
          ...
          >
            {selectedItem?.id === category.id ? (
              ...
              <TextStyled>
                {category.name} ({category.quantity} classes)
              </TextStyled>
            )}
...
export default ClassCategoryDialog;
