...
const VideoCategoryDialog = ({
  ...
  const handleAddNewCategory = async () => {
    ...
    try {
      ...
      const response = await api.post("/exercise/add-category", {
        name: newCategory,
      });
      getListCategory();
      setNewCategory("");
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const response = await api.put(
        `/exercise/edit-category/${selectedItem.id}`,
        {
  ...
  const handleDeleteClick = async (id) => {
    try {
      // eslint-disable-next-line no-unused-vars
      const response = await api.delete(`/exercise/delete-category/${id}`);
      getListCategory();
      setTypeAlert("success");
        ...
        {data.map((category) => (
          ...
          >
            {selectedItem?.id === category.id ? (
              ...
              <TextStyled>
                {category.name} ({category.numVideos} videos)
              </TextStyled>
            )}
            ...
            <Box>
              ...
              {category.numVideos === 0 && selectedItem?.id !== category.id && (
                <IconButton
                  sx={{ color: "#ef3826" }}
...
export default VideoCategoryDialog;
