  ...
  useEffect(() => {
    if (selectedSchedule) {
      const member = listMembers.find(
        (m) => m.id === selectedSchedule?.memberService?.member?.id
      );
      setFormData({
        id: selectedSchedule?.id,
        type: selectedSchedule?.type || "not-select",
        title: selectedSchedule?.title,
        date: dayjs(selectedSchedule?.startDate),
        startTime: dayjs(selectedSchedule?.startDate),
        endTime: dayjs(selectedSchedule?.endDate),
        description: selectedSchedule?.description,
        member: member || "not-select",
      });
      setIsCheckedAllDay(selectedSchedule.title === "Absent All Day");
    }
  }, [selectedSchedule, openDialog, listMembers]);
  ...
  const handleSubmit = async () => {
    ...
    if (!typeError && !timeError) {
      ...
      if (overlapping) {
        ...
        // Xu ly edit schedule
        try {
          const response = await api.put(`/coach/edit-schedule/${selectedSchedule.id}`, {
