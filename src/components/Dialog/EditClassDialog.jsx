...
const EditClassDialog = ({
  ...
  useEffect(() => {
    if (selectedClass) {
      const coach = listCoaches.find(
        (m) => m.coach?.id === selectedClass.coach?.id
      );
      setFormData({
        id: selectedClass.id,
        code: selectedClass.code,
        title: selectedClass.title,
        date: dayjs(selectedClass.startDate),
        startTime: dayjs(selectedClass.startDate),
        endTime: dayjs(selectedClass.endDate),
        coach: coach || "not-select",
        description: selectedClass.description,
        maxParticipants: selectedClass.maxParticipants,
        currentParticipants: selectedClass.currentParticipants,
      });
    }
  }, [selectedClass, openDialog, listCoaches]);
  ...
  const validateField = (field, value) => {
    ...
    switch (field) {
      ...
      case "maxParticipants":
        ...
        } else if (value < formData.currentParticipants) {
          error = "Max participants cannot smaller than current participants.";
        }
  ...
  const handleSubmit = async () => {
    ...
    if (!titleError && !coachError && !maxParticipantsError && !timeError) {
      ...
      const overlapping = formData.coach.schedule?.find((schedule) => {
        ...
        const isSameCoach = formData.coach.coach.id === selectedClass.coach?.id;
        const isDifferentClass = schedule.aclass?.id !== selectedClass.id;
        ...
        return isSameCoach ? isDifferentClass && isOverlap : isOverlap;
      });
      ...
      if (overlapping) {
        ...
        try {
          await api.put(`/class/edit-class/${selectedClass.id}`, {
            title: formData.title,
            categoryId: selectedClass.classCategory?.id,
      ...
      <DialogContent>
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          <Box>
            ...
            <TextFieldItem
              value={selectedClass.classCategory?.name}
              InputProps={{
                readOnly: true,
              }}
            />
          ...
          <Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Box>
                <TitleField>Current participants:</TitleField>
                <TextFieldItem
                  type="number"
              ...
              <Box>
                ...
                <TextFieldItem
                  ...
                  onChange={handleChange}
                  inputProps={{
                    min: formData.currentParticipants,
                    inputMode: "numeric",
                    pattern: "[0-9]*",
                    style: { textAlign: "center" },
                  }}
